"""
MySQL implementation using SQLAlchemy.
Used for local development. Persists data across restarts.
"""
...
class MySQLApplicantRepository:
    """MySQL Applicant repository"""
    ...
    async def update_jee_cgpa(self, applicant_id: int, jee_rank: int, cgpa: float) -> bool:
        ...
        # Assuming JEE rank and CGPA fields exist in Applicant model
...
class MySQLCollegeRepository:
    """MySQL College repository"""
    ...
    async def search_by_eligibility(self, jee_rank: int, cgpa: float, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        # Simplified: assumes colleges have min_jee_rank and min_cgpa fields
...
class MySQLRecommendationRepository:
    """MySQL Recommendation repository"""
    ...
    async def get_job_recommendations(self, applicant_id: int, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        ).order_by(JobRecommendation.recommend_score.desc()).limit(limit).all()
    ...
    def _to_dict(rec):
        return {
            ...
            'score': getattr(rec, 'recommend_score', rec.score),
            'reason': rec.reason,
